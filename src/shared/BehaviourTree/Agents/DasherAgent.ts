...
export const DasherAgent = (handle: BasePart, spawn: BasePart): BehaviourTree => {
    ...
    const selectorTarget = new Selector().addCondition(CondHasBlackboardVariable("target"));
        ...
        const sequenceFollow = new Sequence();
            sequenceFollow.addChild(CondIsTargetCloserThan(distanceToLooseTargetWhenFollowing).case(false, ActionSetBlackboardVariable("target", undefined)));
            sequenceFollow.addChild(CondIsNearSpawn(maxDistanceFromSpawn).case(false, ActionSetBlackboardVariable("target", undefined)))
